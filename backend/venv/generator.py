...
Employees2 = ["{}. {}".format(rows[x][0], rows[x][1])
...
class Worker:
    ...
    def MakePretty(self) -> None:
        ...
        if self.break1 != None:
            break1End = self.break1 + \
                datetime.timedelta(minutes=Constants.BREAKLENGTH)
        ...
        if self.lunch != None:
            lunchEnd = self.lunch + \
                datetime.timedelta(minutes=Constants.LUNCHLENGTH)
...
def BreakLength(shiftLength: int) -> int:
    if shiftLength <= 5:
        ...
        return Constants.BREAKLENGTH + Constants.LUNCHLENGTH
    elif shiftLength > 8:
        return Constants.BREAKLENGTH + Constants.LUNCHLENGTH + Constants.BREAKLENGTH
...
while (flag):
    ...
    MakeBreaks(WorkingToday2[counter], BreakLength(
        ShiftLength(startTime, endTime)))
